	...
	match cmd:
		...
		"navigate-to":
			...
			if params.size() != 2:
				...
				return
			x = float(params[0]) # change to float
			y = float(params[1]) # change to float
		...
		"get-humans":
			...
			for h in humans:
				...
				var face = h.get_node("Face")
				var face_pos = GameState.convert_coordinates_godot2robotics(face.global_transform.origin)
				res.append([h.username, 
							pos.x, 
							pos.y, 
							h.rotation.y, 
							face_pos.x, 
